...
const action = {
  ...
  getWordbooks(categoryIndex) {
    let wordbooks = [];
    Words[categoryIndex]['wordbooks'].forEach((value) => wordbooks.push(value['title']));
    return wordbooks;
  },
  // 获取指定目录、指定单词书的单词列表
  ...
  isCurrentWordbook(categoryIndex, wordbookIndex) {
    ...
    return (currentCategoryIndex == categoryIndex && currentWordbookIndex == wordbookIndex) ? true : false;
  },
  // 设置当前选择的单词书
  ...
  getWordbookProgress(categoryIndex, wordbookIndex) {
    ...
    wordbookProgress[0] = wordbookCount - wordbookProgress[1] - wordbookProgress[2] - wordbookProgress[3];
